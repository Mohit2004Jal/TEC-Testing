...
const validateUser = (req, res) => {
    ...
    if (email !== process.env.EMAIL || password !== process.env.PASSWORD) res.redirect("/login")
...
const updateTankerData = async (req, res) => {
    ...
    try {
        await client.query('UPDATE tanker_info SET tanker_name = $1, factor = $2 WHERE number_plate = $3', [tanker_name, factor, number_plate]);
    }
    catch (err) { console.error(`[${new Date().toLocaleString("en-GB")}] Error updating data: ${err.message}`); }
